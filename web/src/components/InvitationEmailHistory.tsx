...
import { type EmailDelivery } from './invitationDeliveryUtils'
    ...
    return (
        <Stack spacing={1}>
            ...
            {!history.isLoading && !history.error && !history.data?.length && (
                <Typography>Email status unavailable. No sending history has been recorded.</Typography>
            )}
            {(history.data ?? []).map((event, index) => (
                ...
                >
                    <Typography variant="caption">
                        {index === 0 ? 'Latest recorded status · ' : ''}
                        {new Date(event.createdAt).toLocaleString()} · Invitation {event.delivery.invitationId}
                    </Typography>
                    <InvitationDelivery
