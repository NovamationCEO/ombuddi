...
-- =====================================================================
-- Immutable audit trail for privileged organization, seat, role, identity,
-- and invitation lifecycle changes. Invitation secrets are never recorded.
...
CREATE INDEX administrative_events_organization_idx
    ON administrative_events (organization_id, created_at DESC);
CREATE INDEX administrative_events_target_ombuds_idx
    ON administrative_events (target_ombuds_id, created_at DESC)
    WHERE target_ombuds_id IS NOT NULL;

CREATE FUNCTION prevent_administrative_event_changes()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
